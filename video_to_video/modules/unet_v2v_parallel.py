...
class TemporalTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.in_channels = in_channels
        inner_dim = n_heads * d_head
        self.norm = ContextParallelGroupNorm(num_groups=32, num_channels=in_channels, eps=1e-6, affine=True)
        if not use_linear:
    ...
    def forward(self, x, context=None):
        ...
        # [16384, 16, 320]
        if self.use_linear:
            x = rearrange(x, "b c f hw -> (b hw) f c").contiguous()
            x = self.proj_in(x)
        ...
        if self.use_linear:
            ...
            x = self.proj_out(x)
            x = rearrange(x, "(b hw) f c -> b c f hw", b=b).contiguous()
        if not self.use_linear:
            x = rearrange(x, "b hw f c -> (b hw) c f").contiguous()
            x = self.proj_out(x)
            x = rearrange(x, "(b hw) c f -> b c f hw", b=b).contiguous()
...
class TemporalConvBlock_v2(nn.Module):
    ...
    def __init__(self, in_dim, out_dim=None, dropout=0.0, use_image_dataset=False):
        ...
        self.conv1 = nn.Sequential(
            ContextParallelGroupNorm(32, in_dim),
            nn.SiLU(),
            ContextParallelConv3d(in_dim, out_dim, (3, 1, 1), padding=(1, 0, 0)),
        )
        self.conv2 = nn.Sequential(
            ContextParallelGroupNorm(32, out_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
